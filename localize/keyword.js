...
module.exports = {
  ...
  'Respect .gitignore in file picker': '选文件遵守 .gitignore',
  ...
  'Run /init to create a CLAUDE.md file with instructions for Claude': '运行 /init 创建 CLAUDE.md 说明文件',
  'Recent activity': '最近活动记录',
  'No recent activity': '暂无最近活动记录',
  'create a util logging.py that...': '创建一个工具文件 logging.py，用于...',
  'Initialize a new CLAUDE.md file with codebase documentation': '初始化 CLAUDE.md 项目说明',
  'Review changed code for reuse, quality, and efficiency, then fix any issues found.': '检查改动的复用、质量和效率，并修复问题。',
  'Add a new working directory': '添加工作目录',
  ...
  "Copy Claude's last response to clipboard (or /copy N for the Nth-latest)": '复制Claude回复到剪贴板',
  ...
  'View uncommitted changes and per-turn diffs': '查看未提交差异',
  'Diagnose and verify your Claude Code installation and settings': '诊断并验证 Claude Code 的安装和设置',
  'Set effort level for model usage': '设置模型力度',
  ...
  'Manage MCP servers': '管理MCP服务',
  'Edit Claude memory files': '编辑Claude记忆',
  'Show QR code to download the Claude mobile app': '显示Claude手机二维码',
  ...
  'Upgrade to Max for higher rate limits and more Opus': '升级Max获得更多额度',
  ...
  'Run a prompt or slash command on a recurring interval (e.g. /loop 5m /foo, defaults to 10m)': '定时重复运行提示词或命令（如 /loop 5m /foo）',
  ...
  'Claude in Chrome (Beta) settings': 'Chrome Claude设置',
  ...
  'Skip this picker in the future': '以后跳过此选择器',
  ...
  'Set the AI model for Claude Code': '设置AI模型',
  ...
  'Scan your transcripts for common read-only Bash and MCP tool calls, then add a prioritized allowlist to project .claude/settings.json to reduce permission prompts.':
    '扫描记录，把常用只读 Bash 和 MCP 工具加入项目允许列表，减少权限弹窗。',
  'Create a git commit': '创建Git提交',
  'Show current context usage': '显示上下文用量',
  'Configure extra usage to keep working when limits are hit': '配置额外用量，在达到限制后继续工作',
  'Toggle a searchable tag on the current session': '切换当前会话的可搜索标签',
